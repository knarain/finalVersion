export async function generateStaticParams() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      console.warn('generateStaticParams (add): API responded with status', res.status)
    }
  } catch (err) {
    console.error('generateStaticParams (add): failed to fetch from API', err)
  }
  ...
  try {
    ...
    console.error('generateStaticParams (add): failed to read public/albums.json', err)
  }

  console.warn('generateStaticParams (add): using fallback static params')
